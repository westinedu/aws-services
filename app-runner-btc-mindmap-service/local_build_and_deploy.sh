...
# One-shot: package local source -> CodeBuild builds/pushes to ECR -> App Runner deploys that image.
# No local Docker build. Uses a dedicated S3 bucket only as transient source upload (required by CodeBuild for local content),
# and cleans up the uploaded object after each run.

PROFILE="${AWS_PROFILE:-app-runner}"
REGION="${AWS_REGION:-us-east-2}"

SERVICE_NAME="${SERVICE_NAME:-btc-mindmap-service}"    # Also used as App Runner service name
...
cleanup() {
  # Remove local temp and uploaded S3 object to keep the bucket clean.
  rm -rf "$tmpdir"
